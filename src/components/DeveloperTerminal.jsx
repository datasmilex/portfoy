    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/85 backdrop-blur-md overflow-y-auto">
        <motion.div
          ...
          className={`w-[95vw] sm:w-full max-w-2xl max-h-[85vh] sm:max-h-[90vh] bg-[#050505] rounded-none border shadow-2xl overflow-hidden font-mono text-sm flex flex-col transition-colors duration-300 ${
            authStatus === 'error'
        ...
        >
          ...
          <div className="flex items-center justify-between px-4 py-3 bg-[#111] border-b border-white/10 shrink-0">
            ...
            <button
              onClick={onClose}
              className="w-9 h-9 rounded-none bg-[#222] hover:bg-white/20 flex items-center justify-center text-slate-300 hover:text-white transition-colors cursor-pointer shrink-0"
              title="Kapat"
            >
              <X className="w-5 h-5" />
            </button>
          ...
          {!isUnlocked ? (
              ...
              <form onSubmit={handleAuthSubmit} className="w-full max-w-sm flex flex-col items-center gap-4">
                <div className="relative w-full">
                  ...
                  <input
                    ...
                    className={`w-full pl-10 pr-4 py-3 bg-black rounded-none border text-center font-mono tracking-widest text-base focus:outline-none transition-all ${
                      authStatus === 'error'
                        ? 'border-white text-white focus:ring-2 focus:ring-white/50'
                        : authStatus === 'success'
                ...
                <button
                  type="submit"
                  className="w-full py-3 rounded-none bg-white text-black hover:bg-gray-200 font-bold text-xs tracking-wider transition-all shadow-none cursor-pointer flex items-center justify-center gap-2"
                >
                  <span>GİRİŞ YAP</span>
                  <CornerDownLeft className="w-4 h-4 text-black" />
                </button>
              </form>
            </div>
          ) : (
            ...
            <div className="flex flex-col flex-1 overflow-hidden">
              ...
              {/* CLI Input */}
              <div className="flex items-center gap-2 px-4 py-3 bg-[#111] border-t border-white/10 shrink-0">
                ...
                <input
                  ...
                  className="flex-1 bg-transparent text-white focus:outline-none text-xs sm:text-sm placeholder:text-slate-600"
                />
